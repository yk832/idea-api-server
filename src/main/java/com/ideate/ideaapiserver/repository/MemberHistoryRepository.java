...
public interface MemberHistoryRepository extends JpaRepository<MemberHistory, Long> {
    ...
    @Query("select m from MemberHistory m where m.historyType = 'LOGIN' ")
    List<MemberHistory> findByLoginHistoryType();

    @Query("select m from MemberHistory m where m.historyType = 'UPDATE' ")
    List<MemberHistory> findByUpdateHistoryType();
}
